...
from PER import SumTree, PriorExpReplay
...
class Agent():
	...
	def __init__(self, state_size, action_size, random_seed):
		...
		# Replay memory
		if self.prioritized:
			self.memory = PERBuffer(action_size, BUFFER_SIZE, BATCH_SIZE, random_seed)
		else:
	...
	def step(self, state, action, reward, next_state, done, timestep):
		...
		if len(self.memory) > BATCH_SIZE and timestep % LEARN_EVERY == 0:
			for _ in range(LEARN_NUM):
				if self.prioritized:
					experiences,batch_weights, inds = self.memory.sample()
				else:
	...
	def learn(self, experiences, gamma):
		...
		if self.prioritized:
			abs_errors = self.l1_loss(Q_expected, Q_targets)
			critic_loss = (self.memory.b_weights*self.mse_element_loss(Q_expected, Q_targets)).mean()
			self.memory.batch_update(self.memory.tree_idx, self.memory.abs_errors.data.numpy())
		else:
...
class PERBuffer:
	...
	def __init__(self, action_size, buffer_size, batch_size, seed):
		...
		"""
		self.action_size = action_size
		self.memory = PriorExpReplay(buffer_size)
		#self.memory = deque(maxlen=buffer_size)  # internal memory (deque)
		self.batch_size = batch_size
		self.experience = namedtuple("Experience", field_names=["state", "action", "reward", "next_state", "done"])
		self.seed = random.seed(seed)
